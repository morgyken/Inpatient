<div role="tabpanel" id="plan" class="tab-pane fade col-xs-12 col-sm-12 col-md-12 col-lg-12">
    ...
    <form id="care_plan">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6" style="padding: 0 !important;">
            <div class="form-group">
                ...
                <input type="date" name="date_recorded" id="date_recorded" class="form-control"
                       value="{{ \Carbon\Carbon::now('Africa/Nairobi')->toDateString() }}" required>
            </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            <div class="form-group">
                <label>Time</label>
                <input type="text" name="time_recorded" id="time_recorded" class="form-control"
                       value="{{ \Carbon\Carbon::now()->format('H:i a') }}" required>
            </div>
        </div>

        <div class="form-group">
            <label>Nursing Assesment</label>
            <input type="text" name="diagnosis" id="assesment" class="form-control" required>
        </div>
        <div class="form-group">
            ...
            <table class="table table-stripped table-hover" id="care-plan-table">
                ...
                @foreach($nursingCarePlans as $n)
                    <tr id="plan_row_{{ $n->id }}">
                        ...
                        <td>
                            <div class='btn-group'>
                                <button type='button' class='btn btn-primary view-plan' id='{{ $n->id }}'><i
                                            class='fa fa-eye'></i> View
                                </button>
                                <button type='button' class='btn btn-danger delete-plan' id='{{ $n->id }}'><i
                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        if (resp.type === "success") {
                            ...
                            data.map((item, index) = > {
                                return($("#care-plan-table > tbody").append("<tr id = 'plan_row_" + item.id + "'>\
                    ...
                    success: function (resp) {
                        if (resp.type === "success") {
                            ...
                            resp.data.map((item, index) = > {
                                return($("#view-care-plan-table > tbody").append("\
                                        <tr>\
                                            <th>Date & Time</th>\
